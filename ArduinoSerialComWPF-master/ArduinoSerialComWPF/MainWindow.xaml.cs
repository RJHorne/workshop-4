		...
		private void SerialPort_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
		{
			...
			try
			{
				receiveText.Dispatcher.Invoke(
					...
					//	
						receiveText.Text = inData;
                        try {

							potStatus.Value = Convert.ToInt32(receiveText.Text);
                        }
                        catch 
						{
						}
						...
						if(inData == "d\r")
                        {
							MainButton.Fill = (SolidColorBrush)new BrushConverter().ConvertFromString("#FF5F1E1E");
						}

						if (inData == "p\r")
						{
							MainButton.Fill = System.Windows.Media.Brushes.Black;
						}
